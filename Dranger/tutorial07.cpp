  ...
  for(;;) {
	...
	if(is->seek_req)
	{
		...
		if(stream_index>=0)
		{
			...
			seek_target= av_rescale_q(seek_target, time, pFormatCtx->streams[stream_index]->time_base);
		}
		//if(!avformat_seek_file(is->pFormatCtx, stream_index, seek_target, is->seek_flags))
		int res = av_seek_frame(is->pFormatCtx, stream_index, seek_target, is->seek_flags);
		if(res < 0)
		{
