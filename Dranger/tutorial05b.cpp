...
int audio_decode_frame(VideoState *is, uint8_t *audio_buf, int buf_size, double *pts_ptr) {
	...
	for(;;) {
		while(is->audio_pkt_size > 0) {
			data_size = buf_size;
			//len1 = avcodec_decode_audio2(is->audio_st->codec, (int16_t *)audio_buf, &data_size, is->audio_pkt_data, is->audio_pkt_size);
...
void alloc_picture(void *userdata) {
	...
	//vp->c_bmp = (char*)malloc(is->video_st->codec->width * is->video_st->codec->height * 3);
	vp->pictFrame = avcodec_alloc_frame();
	if (vp->pictFrame == NULL)
		printf("Failed to allocate uncompressed video frame buffer.");
	int rawFrameNumBytes_ = avpicture_get_size(PIX_FMT_RGB24, is->video_st->codec->width, is->video_st->codec->height);
	uint8_t *rawFrameBuf_ = (uint8_t*)av_malloc(rawFrameNumBytes_*sizeof(uint8_t));
	if (rawFrameBuf_ == NULL)
		printf("Failed to allocate uncompressed video frame buffer.");
	avpicture_fill((AVPicture*)(vp->pictFrame), rawFrameBuf_, PIX_FMT_RGB24, is->video_st->codec->width, is->video_st->codec->height);
...
int queue_picture(VideoState *is, AVFrame *pFrame, double pts) {
	...
	if(!vp->pictFrame || vp->width != is->video_st->codec->width || vp->height != is->video_st->codec->height)
	...
	if(vp->pictFrame) {
		...
		*/
		...
		//sws_scale(is->decodeCtx_, ((AVPicture *)pFrame)->data, ((AVPicture *)pFrame)->linesize, 0, (pFrame)->height, pict.data, pict.linesize);
		sws_scale(is->decodeCtx_, ((AVPicture *)pFrame)->data, ((AVPicture *)pFrame)->linesize, 0, (pFrame)->height, pict.data, pict.linesize);
...
int stream_component_open(VideoState *is, int stream_index) {
  ...
  case AVMEDIA_TYPE_VIDEO:
	  ...
	  codecCtx->release_buffer = our_release_buffer;
		...
		is->decodeCtx_ = sws_getContext(is->video_st->codec->width, is->video_st->codec->height, is->video_st->codec->pix_fmt, 
								is->video_st->codec->width, is->video_st->codec->height, PIX_FMT_RGB24, SWS_BILINEAR, NULL, NULL, NULL);
...
//	}
	...
	//sprintf(is->filename, "sample.mp4\0");
