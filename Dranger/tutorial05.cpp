...
double get_audio_clock(VideoState *is)
{
	...
	if(hw_buf_size == 0)
		printf("hw_buf_size = 0\n");
	else
		printf("hw_buf_size = 11111\n");
	bytes_per_sec = 0;
...
void video_refresh_timer(void *userdata) {
	...
	if(is->video_st) {
		if(is->pictq_size == 0) {
			...
			//if((int)(actual_delay * 1000 + 0.5) < 9 || (int)(actual_delay * 1000 + 0.5) > 11)
				printf("diff: %f - Delay: %f\n", diff, (actual_delay * 1000 + 0.5));
			/* show the picture! */
			video_display(is);
			...
			is->pictq_size--;
			//fprintf(stderr, "++++++out pictq_size: %d\n", is->pictq_size);
